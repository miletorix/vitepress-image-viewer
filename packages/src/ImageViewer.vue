...
function onTouchMove(e: TouchEvent) {
  if (e.touches.length === 2) {
    ...
    scale.value = Math.min(
      maxScale,
      Math.max(minScale, initialScale * scaleChange)
    )
  }
}
...
.iv-buttons button:disabled {
  opacity: 0.4;
  cursor: not-allowed;
}
.iv-buttons button:disabled svg {
  transform: scale(0.92);
}
.iv-buttons button:hover:not(:disabled) {
  background: rgba(255, 255, 255, 0.08);
  color: #ffffff;
}

.iv-buttons button:active:not(:disabled) {
  transform: scale(0.95);
}

.iv-buttons button:last-child {
  margin-left: 6px;
  background: rgba(242, 63, 66, 0.15);
  color: #f23f42;
}

.iv-buttons button:last-child:hover {
